...
# run azimuth -------------------------------------------------------------
# read in the query
query <- LoadFileInput("out/object/analysis_R45/05_sobj_filtered_harmony_afterQC_diet.rds")
# query <- readRDS(file = "../../data/lymphocytes_sct20_clustering.rds")
...
query <- RunAzimuth(query, reference = "data/reference/azimuth/ref_snRNAseq_human_cerebellum/")

# can also be passed
# query <- RunAzimuth(query, reference = "humancortexref")
saveRDS(query,"out/object/analysis_R45/05_Azimuth_sobj_afterQC.rds")
...
query@reductions$ref.umap@cell.embeddings %>%
  ...
  write_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_CoordUMAP_Azimuth_",reference_id,"_",class,".tsv"))

# save the new annotation from azimuth
query@meta.data %>%
  data.frame() %>%
  rownames_to_column("barcode") %>%
  write_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_meta_Azimuth_",reference_id,"_",class,".tsv"))

# custome plots threshold 0.50 --------------------------------------------
# costume visualization
UMAP_query <- read_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_CoordUMAP_Azimuth_",reference_id,"_",class,".tsv"))
meta_query <- read_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_meta_Azimuth_",reference_id,"_",class,".tsv")) %>%
  mutate(
...
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_score_075.pdf"),height = 4,width = 5)
...
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_ridges_score_075.pdf"),height = 5,width = 5)
...
pdf(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_",cluster_query,"_Heatmap_score_075.pdf"),height = 4,width = 6)
...
# save the topmost proposed annotation per cluster using raw annotation
meta_query %>%
  group_by(cluster.query,predicted.class) %>%
  summarise(n = n()) %>%
  mutate(tot = sum(n)) %>%
  ...
  top_n(n = 1,wt = prop) %>%
  write_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_",cluster_query,"_topAnno_raw.tsv"))
...
pdf(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_",cluster_query,"_HeatmapFilter_score_075.pdf"),height = 4,width = 6)
...
# save the topmost proposed annotation per cluster using robust annotation
meta_query %>%
  filter(robust_class != "uncertain") %>%
  group_by(cluster.query,predicted.class) %>%
  summarise(n = n()) %>%
  ...
  group_by(cluster.query) %>%
  top_n(n = 1,wt = prop) %>%
  write_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_",cluster_query,"_topAnno_robust.tsv"))
...
ggplot()+
  ...
  theme_cowplot()
# ggsave("../out/plot/harmonySkipIntegration_AllSoupX_01000_06000_15_CoordUMAP_AzimuthHumanCortexRef_subclass_UMAP_050.pdf",height = 5,width = 3)
...
# ggsave("../out/plot/harmonySkipIntegration_AllSoupX_01000_06000_15_CoordUMAP_AzimuthHumanCortexRef_subclass_split_UMAP_050.pdf",height = 15,width = 20)

# costum plotting ---------------------------------------------------------
# costume visualization
# umap coordinates of the reference onbject
UMAP_ref <- read_tsv("out/table/analysis_R45/azimuth_cerebellum_CoordUMAP.tsv")

# umap coordinates of the query object
UMAP_ref2 <- read_tsv(paste0("out/table/analysis_R45/05_",sample_id,"_CoordUMAP_Azimuth_",reference_id,"_",class,".tsv"))

# original metadata of the reference object
meta_ref <- read_tsv("out/table/analysis_R45/azimuth_cerebellum_Metadata.tsv") %>%
...
ggplot()+
  ...
  theme_cowplot()
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_UMAP_Cluster_075.pdf"),height = 6,width = 7)
# ggsave("../out/plot/human_motorcortexv1.0.0_subclass_data.combined_annotated_norm_fix_resolution_UMAP_050.pdf",width = 9,height = 6)
...
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_UMAP_Cluster_slit_075.pdf"),height = 6,width = 15)
# ggsave("../out/plot/human_motorcortexv1.0.0_subclass_data.combined_annotated_norm_fix_resolution_UMAP_050_split.pdf",width = 20,height = 15)
...
ggplot()+
  ...
  theme_cowplot()
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_UMAP_class_075.pdf"),height = 6,width = 8)
# ggsave("../out/plot/human_motorcortexv1.0.0_subclass_data.combined_annotated_norm_fix_resolution_UMAP_050_subclass.pdf",width = 8,height = 6)
...
ggplot()+
  ...
  facet_wrap(~var_split,ncol=4)+
  theme(strip.background = element_blank())
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_UMAP_class_split_075.pdf"),height = 6,width = 14)
# ggsave("../out/plot/human_motorcortexv1.0.0_subclass_data.combined_annotated_norm_fix_resolution_UMAP_050_subclass_split.pdf",width = 20,height = 15)
...
sobj <- readRDS("out/object/analysis_R45/04_sobj_filtered_harmony_afterQC.rds")
...
ggplot()+
  ...
  theme_cowplot()
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_UMAPinverse_class_075.pdf"),height = 6,width = 8)
# ggsave("../out/plot/harmonySkipIntegration_AllSoupX_01000_06000_15_CoordUMAP_AzimuthBS_subclass_UMAP_050.pdf",height = 3,width = 5)
...
ggplot()+
  ...
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
ggsave(paste0("out/plot/analysis_R45/05_",sample_id,"_",class,"_",reference_id,"_UMAPinverse_class_split_075.pdf"),height = 6,width = 14)
#ggsave("../../out/image/harmonySkipIntegration_AllSoupX_01000_06000_15_CoordUMAP_AzimuthBS_subclass_split_UMAP_050.pdf",height = 15,width = 20)