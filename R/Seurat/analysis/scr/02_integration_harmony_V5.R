...
# or
# lapply(data.list,function(x){
#   dim(x@assays$RNA@layers$counts)[2]
# }) %>%
#   unlist() %>%
#   sum()
...
# data.combined.all@assays$RNA@layers$counts do not retain the gene names

# old AssayV3
# notice that for the old assay version there is non need to normalize the data before the cell cycle scoring
# sobj_total <- CreateSeuratObject(counts = data.combined.all@assays$RNA@counts,
#                                  project = "test_V5",
#                                  meta.data = data.combined.all@meta.data,
...
# save the object
saveRDS(sobj_total_h,"../out/test_introns/object/data.combined_harmonySkipIntegration_AllSoupX_01000_06000_15_V5.rds")
