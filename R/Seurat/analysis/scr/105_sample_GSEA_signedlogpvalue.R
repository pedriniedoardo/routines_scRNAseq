...
list_ranks <- lapply(results, function(x){
  ...
  ranks <- setNames(x$signed_rank_stats, x$gene)
  ranks
}) 
glimpse(list_ranks)
...
df_tables_GSEA_all %>%
  write_tsv("../out/table/105_df_table_GSEA_KEGG_CD14Mono_ranksiglogpvalue.tsv")
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv(file = "../out/table/105_df_table_GSEA_KEGG_CD14Mono_nonredundant_ranksiglogpvalue.tsv")
...
ggsave("../out/plot/105_GSEA_unbiased_KEGG_CD14Mono_nonredundant_ranksiglogpvalue.pdf",width = 15,height = 10)
...
df_tables_GSEA_all %>%
  ...
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)+
  geom_hline(yintercept = -log(0.05),col="gray",linetype="dashed")
ggsave("../out/plot/105_GSEA_unbiased_KEGG_CD14Mono_ranksiglogpvalue.pdf",width = 15,height = 9)
