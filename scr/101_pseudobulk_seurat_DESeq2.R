...
saveRDS(counts,file = "../out/object/101_counts_mono_pBulk.rds")
saveRDS(colData,file = "../out/object/101_colData_mono_pBulk.rds")
...
pdf("../out/plot/101_heatmap_SampleCluster_filterExp.pdf",width = 10,height = 6)
set.seed(1)
...
ggsave("../out/plot/101_PCA_pseudobulk_filterExp.pdf",width = 6,height = 4)
...
saveRDS(ddsHTSeq_filter,"../out/object/101_ddsHTSeq_pseudobulk_filterExp.rds")
...
# save the table
df_res %>%
  write_tsv("../out/table/101_DE_pseudobulk_filterExp.tsv")

df_res %>%
  filter(symbol %in% c("CFB", "USP18", "ABTB2"))
...
# save the table
df_res_shr %>%
  write_tsv("../out/table/101_DE_pseudobulk_filterExp_shr.tsv")
...
ggsave("../out/plot/101_histogram_pvalue_pseudobulk_filterExp.pdf",width = 6,height = 4)
...
ggsave("../out/plot/101_vulcano_plot_pseudobulk_filterExp.pdf",width = 10,height = 10)
...
plot_volcano_shr %>%
  ...
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../out/plot/101_vulcano_plot_pseudobulk_filterExp_shr.pdf",width = 10,height = 10)
...
df_res %>%
  ...
  theme(strip.background = element_blank()) +
  theme(legend.position = "none")
ggsave("../out/plot/101_MA_plot_pseudobulk_filterExp.pdf",width = 10,height = 10)
...
# PLOT DISPERSION ---------------------------------------------------------
pdf("../out/plot/101_dispersion_plot_pseudobulk_filterExp.pdf",width = 5,height = 5)
