...
# scDblFinder() has two main modes of operation: cluster-based or not. Both perform quite well (see Germain et al., 2021).
...
table(truth=sce2$type, call=sce2$scDblFinder.class)
...
# make a sample conversion to Seurat Object
library(Seurat)
# Convert to Seurat object
# seurat_object <- as.Seurat(sce,assay = "counts")
se <- CreateSeuratObject(counts = assay(sce),
                         meta.data = data.frame(colData(sce)))

# make as ident the cluster variable
Idents(se) <- "cluster"

set.seed(123)
sce_test <- scDblFinder(GetAssayData(se, slot="counts"), clusters=Idents(se))

# port the resulting scores back to the Seurat object:
se$scDblFinder.score <- sce_test$scDblFinder.score
se$scDblFinder.class <- sce_test$scDblFinder.class

# make sure the result from both object is the same
# set.seed(123)
# sce <- scDblFinder(sce, clusters="cluster")

# compare the individual objects
table(truth=sce$type, call=sce$scDblFinder.class)
table(truth=se$type, call=se$scDblFinder.class)

# compare the two objects
table(sce$scDblFinder.class, se$scDblFinder.class)
...
# 1.5.9Can this be used with scATACseq data?
# Yes, see the scATAC vignette specifically on this topic.
                                                                                                  
# 1.5.10Should I run QC cell filtering before or after doublet detection?
...
# 1.5.10.1What about ambiant RNA decontamination?
...
# 1.5.11Can I combine this method with others?
